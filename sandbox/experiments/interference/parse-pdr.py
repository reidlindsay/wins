...
"""
Parse PDR data from trace files.
    ...
    for e in trace.events:
        ...
        if (obj=="80211N") and (evt=="SND"):
            if (root==curreth):
                ...
                sendbuffer[pkt] = {'eth': root, 'detect':None}
                sendbuffer[pkt].update({'error':None, 'ncoll': None})
        ...
        if (obj=="80211N") and (evt=="RCV"):
            assert hasfield(e, 'dot11n-error', *cifargs)
            ncoll = len(eval(e['cif-collision']))
            if (pkt in sendbuffer):
                assert hasfield(e, 'dot11n-error')
                assert (sendbuffer[pkt]['detect'])
                assert (sendbuffer[pkt]['error'] is None)
                sendbuffer[pkt]['error'] = eval(e['dot11n-error'])
                sendbuffer[pkt]['ncoll'] = ncoll
        # check for 80211N DRP
        if (obj=="80211N") and (evt=="DRP"):
            assert hasfield(e, 'drop', *cifargs)
            ncoll = len(eval(e['cif-collision']))
            if (pkt in sendbuffer) and sendbuffer[pkt]['detect']:
                # assume header decoding failed
                sendbuffer[pkt]['error'] = True
                sendbuffer[pkt]['ncoll'] = ncoll
    # calculate PDR data from sendbuffer
    assert (G is not None)
    for p, d in sendbuffer.items():
        assert hasfield(d, 'eth', 'detect', 'error', 'ncoll')
        eth, detect = d['eth'], d['detect']
        error, ncoll = d['error'], d['ncoll']
        maxcoll = options.maxcoll
        if detect and (eth!=curreth):
            # create new data point
            assert (ncoll is not None)
            assert (error is not None)
            if error: pdr = 0.0
            else:     pdr = 1.0
            dp = {'x': G, 'y': pdr, 'ndata': 1}
...
def parse_pdr():
    ...
    defaultparam = {'xlabel': "G", \
                    'ylabel': "PDR", \
                    'title': "Packet-Delivery Ratio vs. Workload", \
                    'label': None, \
                    'source': None, \
                    'format': None}
    ...
    for k in range(numtraces):
        ...
        sys.stderr.write("Parsing PDR data from %s ...\n"%(tfile))
