...
"""
Simulate interference scenario with ALOHA style interferers.
...
def get_topology(options):
    ...
    sirmin = options.sirmin
    sirmax = options.sirmax
    sir = np.linspace(sirmax, sirmin, numnodes)
    # channel parameters
    alpha      = options.alpha
...
def run_experiment(options):
    ...
    frametime = 2*duration
    srate = 1.0/frametime               # sender rate in frames/second
    sdelay = 1.0/srate
...
def main():
    ...
    parser.add_option("", "--sirmax", dest="sirmax", type="float", \
            default=10, help="Set maximum SIR value for simulation [default=%default]")
    parser.add_option("", "--sirmin", dest="sirmin", type="float", \
            default=10, help="Set minimum SIR value for simulation [default=%default]")
