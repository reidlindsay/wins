...
"""
Parse PDR of RBAR from trace files.

Revision Info
=============
* $LastChangedBy: mandke $
* $LastChangedDate: 2011-12-14 14:14:45 -0600 (Wed, 14 Dec 2011) $
* $LastChangedRevision: 5374 $
...
def parse_info(options, trace):
    ...
    for p,d in sendbuffer.items():
        ...
        if (d['rxts'] is None):
            x, y = d['snr'], 0       # log with zero (not delivered)
        else:
            x, latency = d['snr'], d['rxts']-d['txts']
            y = 1.0                  # log with one (delivered)
        dp = {'x': x, 'y': y, 'ndata': 1}
        data.append(dp)
